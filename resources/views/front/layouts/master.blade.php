...
<body>
@php
  // Check if the current locale is one of the RTL languages
  $isRtl = in_array(app()->getLocale(), ['ar', 'krd']);
@endphp
                    ...
                    <div class="nav-outer">    
                        <nav class="nav main-menu">
                            <ul class="navigation">
                                ...
                                <li class="{{ (Request::is('/')||Route::is('home_1')||Route::is('home_2')||Route::is('home_3')||Route::is('home_4')) ? 'current' : ''; }}"><a href="{{ route('home_3') }}">{{ __('Home') }}</a>
                                    <!-- <ul > -->
                                ...
                                @endphp
                                @endforeach
                                <li class="dropdown"><a href="javascript:;">{{ __('info') }}</a>
                                    <ul >
 ...
 @if($menu_arr['Services'] == 'Show')
    <li class="{{ Request::is('services') ? 'current' : ''; }} dropdown">
        ...
        <ul>
            @foreach($services_category as $category)
                ...
                @if(count($category->subcategories) > 0)
                    <li class="dropdown">
                        <a href="{{ route('home') }}" class="non-clickable">
                        @if(session('sess_lang_code') == 'ar')
                        {{ $category->name_ar }} <!-- Arabic name -->
                    @elseif(session('sess_lang_code') == 'krd')
                          {{ $category->name_krd }} <!-- Kurdish name -->
    ...
    <li>
        @if(!empty($subcategory->url))
            <a href="{{ $subcategory->url }}" target="_blank">

                @if(session('sess_lang_code') == 'ar')
                        {{ $subcategory->name_ar }} <!-- Arabic name -->
                    @elseif(session('sess_lang_code') == 'krd')
                          {{ $subcategory->name_krd }} <!-- Kurdish name -->
                    @else
                             {{ $subcategory->name_en }} <!-- English name -->
                    @endif
            
            </a>
        @else
            <a href="{{ route('servicecat', ['category_name' => $category->name_en, 'sub_cat_id' => $subcategory->id]) }}" onclick="submitForm('{{ $subcategory->id }}')">
...
<form id="subcategoryForm" action="{{ route('services') }}" method="POST" style="display: none;">
    @csrf
...
<!-- ******************************************************* -->
  ...
  <li class="{{ Request::is('products') ? 'current' : '' }} dropdown">
                    ...
                    <ul>
                        @foreach($category->subcategories as $subcategory)
                            <li>
                                ...
                                @else
                                    <a href="{{ route('productcat', ['category_name' => $category->name_en,'subcategory_name' => $subcategory->name_en, 'sub_cat_id' => $subcategory->id]) }}">
                                        @if(session('sess_lang_code') == 'ar')
                                            {{ $subcategory->name_ar }} <!-- Arabic name -->
                                        @elseif(session('sess_lang_code') == 'krd')
                                            {{ $subcategory->name_krd }} <!-- Kurdish name -->
...
<!-- ************************************************************************ -->
                                ...
                                <!-- @if($menu_arr['Services'] == 'Show')
                                <li class="{{ Request::is('services') ? 'current' : ''; }} dropdown">
                                    <a href="{{ route('services') }}">{{ __('products') }}</a>
                                </li>
                                @endif -->
    ...
    <div class="auto-container">
        <div class="inner-container">
            ...
            <div class="logo">
                <a href="{{ route('home') }}">
                    @if(Route::is('home_4'))
                        <img src="{{ asset('uploads/'.$global_setting->logo) }}" alt="{{ env('APP_NAME') }}" style="width: 400px; height: auto; max-height: 150px; object-fit: contain;">
                    @else
                        <img src="{{ asset('uploads/'.$global_setting->logo_sticky) }}" alt="{{ env('APP_NAME') }}" style="width: 400px; height: auto; max-height: 100px; object-fit: contain;">
                    @endif
                </a>
            </div>
...
@endphp 
  <a href="tel:+{{ $global_setting->footer_phone }}"
            ...
            <div class="widgets-section">
                <div class="auto-container">
                    <div class="row">
                        ...
                        <div class="footer-column col-xl-5 col-lg-4 col-md-12">
                            <div class="footer-widget about-widget">
                                <div class="widget-content">
                                    <div class="logo"><a href="{{ route('home') }}"><img src="{{ asset('uploads/'.$global_setting->logo) }}" alt="{{ env('APP_NAME') }}"></a></div>
                                    <div class="text">{{__('footer_text')}}</div>
                                    @if($global_setting->twitter!=''||$global_setting->facebook!=''||$global_setting->linkedin!=''||$global_setting->instagram!=''||$global_setting->youtube!=''||$global_setting->pinterest)
                        ...
                        <!--        <h5 class="widget-title">{{ __('Explore') }}</h5>-->
                        <!--        <div class="widget-content">-->
    ...
    <div class="widget links-widget">
        <h5 class="widget-title">{{ __('Explore') }}</h5>
        <div class="widget-content">
            <ul class="user-links inline-links">
                <li><a href="{{ route('about') }}">{{ __('About') }}</a></li>
                <li><a href="{{ route('faqs') }}">{{ __('FAQ') }}</a></li>
                <li><a href="{{ route('contact') }}">{{ __('Contact') }}</a></li>
            </ul>                                
                        ...
                        <div class="footer-column col-xl-3 col-lg-4 col-md-6 col-sm-12">
                            <!-- <div class="widget newsletter-widget">
                                <h5 class="widget-title">{{ __('Newsletter') }}</h5>
                                <div class="widget-content">
                                    <div class="text">
                                    
                                    @if(session('sess_lang_code') == 'ar')
                                    {!! str_replace(["<p>", "</p>"], ["", ""], clean(nl2br($global_setting->footer_subscriber_text_ar))) !!}
                                    @elseif(session('sess_lang_code') == 'krd')
                                     {!! str_replace(["<p>", "</p>"], ["", ""], clean(nl2br($global_setting->footer_subscriber_text_krd))) !!}
                                     @else
                                    
                                     {!! str_replace(["<p>", "</p>"], ["", ""], clean(nl2br($global_setting->footer_subscriber_text))) !!}bb
                                     @endif
    ...
    s1.async=true;
    s1.src='https://embed.tawk.to/{{ $global_setting->tawk_live_chat_property_id }}/1i9vub867';
    s1.charset='UTF-8';
...
<style>
/* Default styles for globe icons */
.globe, .globe-black {
    width: 40px; /* Adjust the default size as needed */
    height: auto; /* Maintain aspect ratio */
}

/* Media query for smaller screens (adjust the max-width as per your design needs) */
@media (max-width: 768px) {
    .globe, .globe-black {
        width: 0; /* Reduce size for smaller screens */
    }
}

/* Additional media queries if necessary for smaller devices */
@media (max-width: 480px) {
    .globe, .globe-black {
        width: 0; /* Further reduce size for very small screens */
    }
}
</style>
</html>
