    ...
    <div class="auto-container">
        <div class="title-outer">
            ...
            @if(session('sess_lang_code') == 'ar')
                    {{ $global_other_page_items->page_privacy_title_ar }}
                        @elseif(session('sess_lang_code') == 'krd')
                        {{ $global_other_page_items->page_privacy_title_krd }}
                        @else
                        {{ $global_other_page_items->page_privacy_title }}
                      @endif
...
<section class="blog-details">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
            @if(session('sess_lang_code') == 'ar')
            {!! clean($global_other_page_items->page_privacy_content_ar) !!}
                        @elseif(session('sess_lang_code') == 'krd')
                        {!! clean($global_other_page_items->page_privacy_content_krd) !!}
                        @else
